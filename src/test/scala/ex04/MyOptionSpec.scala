...
class MyOptionSpec extends FunSpec with Matchers {
  ...
  describe("MyOption#foreach") {
    it("should be called this MyOption value if it is not empty") {
      var called = false
      MySome(true).foreach(called = _)
      called shouldBe true
    }
    it("should be not called the default value if it is empty") {
      var called = false
      MyNone.foreach(called = _)
      called shouldBe false
    }
  }

  describe("MyOption#forall") {
    it("should return true if it is not empty") {
      MySome(4).forall(_ % 2 == 0) shouldBe true
    }
    it("should return false if it is empty") {
      MyNone.forall(_ => true) shouldBe true
    }
  }

  describe("MyOption#exist") {
    it("should return true if it is not empty") {
      MySome(4).exists(_ % 2 == 0) shouldBe true
    }
    it("should return false if it is empty") {
      MyNone.exists(_ => true) shouldBe false
    }
  }
}